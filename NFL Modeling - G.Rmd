...
Team names/abbs
```{r}
team_abbr <- nflreadr::load_teams() %>% select(team_abbr) %>% pull()
...
NFL_long <- NFL_rf_results %>%
  select(team, pass_prop, run_prop) %>% 
  pivot_longer(
    cols = c(pass_prop, run_prop),
    names_to = "type",
    values_to = "prop"
  )

ggplot(NFL_long, aes(x = team, y = prop, fill = type)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.8) +
  scale_fill_manual(values = c("pass_prop" = "blue", "run_prop" = "red"),
                   labels = c("Pass", "Run"))
...
NFL_long %>%
  ggplot(aes(x = team, y = prop, fill = type)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.8) +
  nflplotR::scale_fill_nfl() +
  nflplotR::geom_nfl_logos(aes(team_abbr = team), width = 0.05) +  # This replaces scale_x_nfl
  scale_y_continuous(labels = scales::percent) +
  theme_minimal() +
  theme(axis.text.x = element_blank()) +  # Remove x-axis text since we have logos
  labs(title = "Pass vs Run Play Distribution by Team",
       y = "Proportion",
       fill = "Play Type")
```
...
ggplot(data = NFL_rf_results, aes(x = full_ru_acc, y = full_pa_acc)) +
  ...
  annotate("text", x = 0.95, y = 0.95, size = 3.5, label = "Above avg. run acc,
           Above avg. pass acc") +
  annotate("text", x = 0.95, y = 0.55, size = 3.5, label = "Above average run acc,
           Below avg pass acc") +
  annotate("text", x = 0.55, y = 0.55, size = 3.5, label = "Below average run acc,
           Below avg pass acc") +
  annotate("text", x = 0.55, y = 0.95, size = 3.5, label = "Below average run acc,
           Above avg pass acc") +
