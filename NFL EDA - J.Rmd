...
filtered_data <- filtered_data %>%
  mutate(run_pass_ind = case_when(
    dropbackType %in% c("TRADITIONAL", "SCRAMBLE") | str_detect(dropbackType, "ROLLOUT") ~ 0,  # Pass
    dropbackType %in% c("QB_SNEAK", "DESIGNED_RUN") ~ 1,                                      # Run
    TRUE ~ NA_real_                                                                           # Safety fallback
  ))
...
#Find an observation in the plays data that has a man in motion
colnames(track1_df)
```

remove na values from the offensive formation 
receiver alignment 
...
#Create a data frame for experimenting 
...
#Calculate the sum of the distance traveled for each player during each play
#group by game, player, play 
#sum total distance per play 
#create new var that is total instances of those groupings n()/
#Get average distance traveled on 
#mean() of the distance 

#Create a dummy data frame to look at data more easily
df7_dummy <- filtered_df7 %>%
  select(playId,gameId,frameId,displayName,dis, event)
...
mean_distance_df <- filtered_df7 %>%
  ...
  summarize(mean_dis = mean(dis, na.rm = TRUE), .groups = "drop") # Avoids grouped output
