...
#Create a data frame with just the predictors to tune labmda values
nfl_trimmed <- nfl_df %>%
  select(-game_id, -week, -play_id, -possession_team)

#Make the run pass variable a factor
nfl_trimmed$run_pass <- as.factor(nfl_trimmed$run_pass)
...
nfl_trimmed[bool_cols] <- lapply(nfl_trimmed[bool_cols], as.numeric)

#Remove NA values
nfl_trimmed <- nfl_trimmed %>%
  drop_na()
...
lasso_rec <- recipe(run_pass~., data=nfl_trimmed) %>%
...
#Use the first 6 weeks of data as training and the last 3 as testing
nfl_train <- nfl_trimmed %>%
  filter(week<=6)

#Perform 10-fold cross-validation to get an estimate of test MSE for each
#labmda in our grid
nfl_10fld_cv <- vfold_cv(nfl_trimmed, v=10)
...
final_lasso_model <- fit(final_lasso_workflow, data=)
