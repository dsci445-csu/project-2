...
We began with multiple linear regression as a baseline modeling approach to establish a clear and interpretable benchmark for predicting calories burned in 30 minutes. Three models were fit, one using only exercise predictors, one using only diet predictors, and one combining both sets of variables.
...
The exercise only model explained approximately **29% of the variability** in calorie burn, indicating that workout duration, heart rate measures, repetitions, sets, and workout characteristics capture meaningful but incomplete information. In contrast, the diet only model performed poorly, explaining only about **2.5% of the variability**, suggesting that diet variables alone are insufficient for predicting short-term calorie burn during a workout.

The combined model modestly improved performance, explaining roughly **31% of the variability**, with a slight reduction in RMSE relative to the exercise-only model. While this indicates that diet variables add some information, the improvement is marginal, reinforcing the idea that calorie burn during a workout is driven primarily by exercise predictors.

Diagnostic plots revealed clear violations of linear model assumptions, including heteroskedasticity and non-normal residuals. These patterns suggest that the relationship between predictors and calorie burn is not purely linear and that important nonlinear effects remain unmodeled. As a result, while linear regression provides interpretability and a useful baseline, it is limited in its ability to capture the complex structure present in the data.
...
In this project, we investigated calorie burn during a 30-minute workout using a large, complex dataset and a sequence of increasingly flexible modeling approaches. We began our analysis with exploratory data analysis, which revealed strong associations between calorie burn and exercise related variables such as session duration, heart rate measures, repetitions, and sets, while diet related variables showed much weaker linear relationships.

Linear regression models provided a useful baseline, demonstrating that exercise predictors account for substantially more variation in calorie burn than diet predictors. However, the clear diagnostic violations highlighted the limitations of linear assumptions in this context.

Generalized additive models (GAMs) substantially improved performance by capturing nonlinear relationships for calorie burn. When modeling calories burned per 30 minutes, the GAM achieved a dramatic increase in modeling power, explaining over **96% of the variability**, and revealed meaningful nonlinear effects of session duration, heart rate measures, and workout intensity. While diet variables were statistically significant in the combined GAM, their contribution to overall model fit was minimal, reinforcing the exercise related predictors' dominance in the models.

Tree-based methods further supported these findings. The regression tree identified workout intensity and body composition as primary drivers of calorie burn. The random forest provided the strongest predictive performance overall, explaining over **90% of the variance**. These models demonstrated that calorie burn is governed by complex interactions and nonlinear effects that are difficult to capture with parametric models alone.

Across all the models, cross-validation results showed that increasing model flexibility improved predictive accuracy without substantial overfitting. Taken together, our findings indicate that calorie burn during short workout sessions is primarily determined by exercise intensity, workout structure, with diet playing a fairly small role. This work highlights the importance of using flexible modeling approaches when analyzing high-dimensional fitness data and underscores the value of predictive performance in applied machine learning research.
